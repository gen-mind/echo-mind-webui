<script lang="ts">
	...
	import { updateAssistant, getLLMs } from '$lib/apis/echomind';
	import type { Assistant, UpdateAssistantRequest, LLM } from '$lib/apis/echomind';

	const i18n = getContext('i18n');
	const dispatch = createEventDispatcher();

	export let show = false;
	export let item: Assistant | null = null;
	...
	$: if (item && show) {
		...
		starterMessages = [...(item.starter_messages || [])];
	...
	async function handleSubmit() {
		...
		loading = true;
		try {
			const data: UpdateAssistantRequest = {
				name: name.trim(),
				llm_id: llmId,
			...
			const result = await updateAssistant(localStorage.token, item.id, data);
			toast.success($i18n.t('Assistant updated successfully'));
			dispatch('updated', result);
			show = false;
		} catch (error) {
			toast.error(error?.toString() || $i18n.t('Failed to update assistant'));
		} finally {
			loading = false;
		}
	}
...
<Modal size="lg" bind:show>
	<div class="p-6 max-h-[90vh] overflow-y-auto">
		...
		<div class="flex justify-between items-center mb-6">
			<h3 class="text-lg font-medium dark:text-white">{$i18n.t('Edit Assistant')}</h3>
			<button
